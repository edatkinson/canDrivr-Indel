...
if __name__ == "__main__":
    ...
    dataset = '/Users/edatkinson/Repos/canDrivr-Indel/independentTesting/independent_test_set_combined_copy.csv_variant_effect_output_all.txt'
    uniprotIDs, df3 = getUniprotIDs(dataset)
    df = pd.merge(df3, uniprotIDs, on = "gene", how = "right").drop("uniprot_conversion", axis =1)
    df.drop(columns=['driver_stat'], inplace=True)
    df.rename(columns={'R':'driver_stat'}, inplace=True)
    df.to_csv('/Users/edatkinson/Repos/canDrivr-Indel/canDrivr/Features/ProteinStructure/vep_indtest_prot.tsv', sep='\t', index=None)
